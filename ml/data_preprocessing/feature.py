...
def features(df):
    ...
    df = df.dropna(subset=["return_1","return_5","return_10","target"]).reset_index(drop=True)
    return df
...
def splitting_and_processing(df,seq_length=30,include_test=True):
    ...
    X_seq,y_seq=create_sequences(
        X_scaled,y_scaled,seq_length
    )

    return X_seq,y_seq,X_scaler,y_scaler

def tensor_conversion(X_train,y_train,X_test=None,y_test=None):
...
def create_dataloaders(X_train,y_train,X_test=None,y_test=None,batch_size=32):
...
def build_pipeline(csv_path,batch_size=32,seq_length=30,include_test=True):
    ...
    X_train,y_train=tensor_conversion(
        X_train_seq,
        y_train_seq
    )
